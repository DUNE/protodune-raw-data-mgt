...
\begin{abstract}
\noindent  Data Quality Monitoring (DQM) and Prompt Processing necessary for its implementation
are important work areas  in the Single-Phase \pd (CERN experiment \expname).
This note describes design parameters of the Prompt Processing system, and one particular design option for such system
...
the basic requirements as per  DocDB 1811 \cite{docdb1811}. A data-driven design is proposed with
a focus on dataflow and throughput which satisfies these requirements, and is based on dynamically prioritized
jobs queue.
\end{abstract}
...
other US and international locations. All or most links in the data transmission chain will be based on F-FTS \cite{fts}.
...
troubleshooting and other applications.
...
The situation is further complicated by the need to handle three data streams (\ref{sec:streams})
in certain stages of processing: LArTPC, \textbf{BI} and \textbf{CRT}. The information contained
in the two latter streams will be absolutely necessary for event reconstruction, which is one of the final
stages in the processing chain. These challenges are been analysed at the time of writing and the
final decision will be made at a later point. There may be potentially a third possibility, where
some of the initial stages of the processing chain are located near the experiment, while
advanced resontruction takes place at CERN Tier-0 facilities.
...
\item A database will be used to manage the workflow and marshal data to jobs, to keep and

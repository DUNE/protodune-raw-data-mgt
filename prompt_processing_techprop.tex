...
visual and quantitative monitoring of the detector performance. The \textbf{Reco} stage is undoubtedly useful
but it will require an order of magnitude more resources to implement, which will reduce resource availability
for other part of \PP. It also requires a more sophisticated orchestration of data which would include
merging with \textbf{BI} and \textbf{CRT} streams and at least rudimentary calibrations. It is unlikelty
for these reasons that the \textbf{Reco} stage will be implemented in \PP but it's kept in the following
description for sake of completeness.

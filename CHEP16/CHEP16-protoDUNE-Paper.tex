...
the construction of the large-scale principal DUNE detectors at the Sanford Underground Research Facility.
...
\noindent
Internals of the DAQ and Online Monitoring in \pd are outside of the scope of this document. We'll note
however one detail of the online architecture which is: the ``outer layer'' of DAQ i.e. the system where the
data exists in a format suitable for writing into files consists of a few \textit{Event Builders} -- machines which
assemble complete readout frames from data fragments obtained from an inner layer of DAQ,
the \textit{Board Readers}. The Event Builders write the data  to the \textit{Online Buffer}, a storage system
which is attached to the \pd DAQ.  At the time of writing the plan is to implement the buffer based on a COTS
storage system from DELL. A backup (auxiliary) option is a XRootD cluster that may be deployed on existing
...
in the raw data management scheme. This is motivated by the experience and architecture of the LHC experiments
...
Fermi FTS system which transports data between a few endpoints. One instance is re

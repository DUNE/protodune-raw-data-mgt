...
In order to provide the necessary precision for reconstruction of the ionization patterns in the LArTPC, the
...
\noindent
Another factor is the relatively slow drift velocity of electrons in Liquid Argon, which is of the order of millimeters per microsecond,
depending on the drift volume voltage and other parameters. This leads to a substantial readout window
(of the order of milliseconds) required to collect all of the ionization in the Liquid Argon volume due the event of interest.
All these factor combined lead to a considerable amount of data per readout window.
A LArTPC event may be compared to a stack of thousands of digital  photographs.

There are data reduction (compression) techniques that may be applicable to \pd raw data in order to reduce its size. 
At the time of writing it is assumed that only the lossless algorithms will be applied during the compression of the \pd raw data.
One example is the Huffman algorithm, while others are also considered.
...
%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[tbh]
\centering\includegraphics[width=\linewidth]{figures/placeholder.JPG}

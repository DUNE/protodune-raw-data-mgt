...
storage location which is effectively a ``dropbox'', i.e.\,files are picked up
...
tape storage and from which it is transmitted to FNAL 
EOS will also be used to provide input to the prompt processing system
in order support the Data Quality Moniotring (DQM) in \pd as explained below.

\section{Data Quality Monitoring}
Data Quality Monitoring (DQM) plays an imporant role in \pd.
It consists of two parts,  the low latency Online Monitoring (OM)
...
Conceptually, they both serve the same purpose of generating time-critical
DQM information on a short time scale needed to ascertain the condition
and performance of both the detector and the DAQ and are complementary to each other. 
The benchmark turnaround time for ``prompt processing'' jobs 
is set at 10\,min for reference purposes, with actual numbers depending on
finalized content of the payload.

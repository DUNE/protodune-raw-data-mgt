...
(e.g. Huffman algorithm and others).
 ...
 As explained in Sec.\,\ref{sec:np04_data_rate} below,
...
petabytes (including commissioning runs with cosmic rays).
...
of this document but we'll note a few details relevant for the topic at hand.
...
suitable for writing into files consists of 10
...
\item Two front-end nodes (DELL R620)
\item Storage Controller (DELL MD3420)
\item Four expansion shelves (DELL MD1220)
\item A total of 120 HDDs
\end{itemize}
...
In the following, we shall consider the handling and management of the data after it is committed
to the Online Buffer.  Conceptual diagram of the raw data flow in \pd is presented in
Fig.\ref{fig:raw_concept}. It shows the general logic of data flow and also reflects
the central role of the EOS system at CERN (the high-performance distributed disk storage
...
CERN (CASTOR) and from which it is transmitted to a number of endpoints including principal data
centers such as FNAL and others. It is also used to provide input to the prompt processing system
which support the Data Quality Moniotring in \pd.

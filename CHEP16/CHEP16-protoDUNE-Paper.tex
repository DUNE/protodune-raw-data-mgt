...
leveraging the expertise and components created at Fermilab, in a broader context of integration with the
systems at other National Laboratories in the US as well as at CERN and other European sites. 
\end{abstract}
...
single-phase LArTPC (NP04). This detector  functions without amplification in the medium
...
DUNE/\pd design has the following characteristics which are common with other recent detectors based on the same technology:
\begin{itemize}
...
\noindent
Another factor is the relatively slow drift velocity of electrons in Liquid Argon, which is of the order of mm/$\mu$s.
This leads to a substantial readout window width
(of the order of milliseconds). All this results in a considerable amount of data per single event readout.
A LArTPC event may be compared to a stack of thousands of digital  photographs. A photon detector
integrated into the TPC will also record data but its rate and volume are expected to be quite small
compared to the TPC data so it won't be addressed here.
...
petabytes (including commissioning runs with cosmic rays) (see Sec.~\ref{sec:np04_data_rate}).
...
\item Average rate (including injestion to mass storage): 3.3\,GB/s
\item Total data volume during the projected run period: 1.2\,PB
...
%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[tbh]
\centering\includegraphics[width=\linewidth]{figures/protoDUNE_data_flow_2017_v1.png}
\caption{\label{fig:raw_concept}Conceptual diagram of the flow of raw data in \pd}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%

\noindent
Internals of the DAQ and Online Monitoring in \pd are outside of the scope of this document but we'll note
a few details relevant for the topic at hand. The DAQ has of a few interconnected layers. The ``outer layer''
i.e. the system where the data is put together in a format suitable for writing into files consists of a
few \textit{Event Builders} -- computers which assemble complete readout frames from data
fragments obtained from the inner layer of DAQ, the \textit{Board Readers}. The Event Builders write the
data  to the \textit{Online Buffer}, a storage system which is attached to the \pd DAQ.  At the time of
writing the plan is to implement the buffer based on a COTS storage system from DELL.
A backup (auxiliary) option is a XRootD \cite{xrootd} cluster that may be deployed on existing
CERN hardware, and a small prototype has been tested.

In the following, we shall consider the handling and management of the data after it arrives to the Online Buffer. 
Conceptual diagram of the raw data flow in \pd is presented in Fig.\ref{fig:raw_concept}. It shows the general logic
of data flow and also reflects the central role of the EOS system at CERN (the high-performance distributed disk storage
\cite{eos}) in the raw data management scheme.
This is motivated by the experience and architecture of the LHC experiments
as well as the \pd data characteristics presented above in Sec.~\ref{sec:np04_data_rate}.
The elements in this diagram which contain ``FTS'' in their label correspond to components and instances of the
\textit{Fermi FTS} system \cite{fts} which transports data between a few endpoints. One instance is re

The EOS system serves as the staging area from which the data gets committed to archival tape storage at
...
We highly recommend the which is included with this documentation.
...
\begin{figure}[h]
\begin{minipage}{14pc}
\includegraphics[width=14pc]{figures/placeholder.JPG}
...
\begin{figure}[h]
\includegraphics[width=14pc]{figures/placeholder.JPG}\hspace{2pc}%
